...
String pulseViewCommandHandlerCallback (int argc, char *argv [], telnetConnection *tcn) {
    ...
    if (argv0is ("SESSION START")) {
        ...
        if (pOscSharedMemory) {
            ...
            pOscSharedMemory->gpio1 = (gpio_num_t) 255; // invalid, has to be set later with CHAN command
            pOscSharedMemory->gpio2 = (gpio_num_t) 255; // invalid, has to be set later with CHAN command, if used
    ...
    if (argv0is ("*IDN?")) {
        ...
        gpio_num_t gpio1, gpio2;
        pOscSharedMemory->gpio1 = (gpio_num_t) 255;
        pOscSharedMemory->gpio2 = (gpio_num_t) 255;
        switch (argc) {
            case 3:   gpio2 = (gpio_num_t) atoi (argv [2]);
                      if (gpio2 < 0 || gpio2 > 39) {
                          return String ("Wrong channel ") + argv [2];
                      }
                      pOscSharedMemory->gpio2 = gpio2;
            case 2:   gpio1 = (gpio_num_t) atoi (argv [1]);
                      if (gpio1 < 0 || gpio1 > 39) {
