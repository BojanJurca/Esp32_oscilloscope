...
ftpServer ftpSrv ("0.0.0.0",                  // start FTP server on all available ip adresses
...
String httpRequestHandler (String httpRequest, WebSocket *webSocket) {  
  ...
  // handle WS (WebSockets) protocol requests, return "" when done
  if (httpRequest.substring (0, 21) == "GET /runOscilloscope ") { // called from oscilloscope.html
...
webServer webSrv (httpRequestHandler,         // a callback function tht will handle HTTP request that are not handled by webServer itself
